...
**_Keywords_: Greedy, Priority Heap, Shortest path from a single source vertex to all other vertices, Go to nearest vertex and mark it covered, Init: distances[V], visited[V],pq = [(0, source)] (i.e., (curr_dist_to_u+edgeweight_to_v, v)),distances[source] = 0, do not set visited[source]=True (heap may contain multiple edges to a vertex, use visited with meaning 'finalized')**
